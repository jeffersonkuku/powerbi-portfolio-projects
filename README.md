# Power BI Portfolio

[English](#english) · [Français](#francais)

---

<a id="english"></a>

# 🇬🇧 English

> **Business Intelligence projects focused on financial steering, procurement performance and decision-support reporting.**

![Power BI](https://img.shields.io/badge/Power%20BI-Data%20Visualization-F2C811?logo=powerbi&logoColor=000)
![Power Query](https://img.shields.io/badge/Power%20Query-ETL-217346)
![DAX](https://img.shields.io/badge/DAX-Analytics-0078D4)
![Excel](https://img.shields.io/badge/Excel-Data%20Source-217346?logo=microsoft-excel&logoColor=white)
![Portfolio](https://img.shields.io/badge/Portfolio-Anonymized-6C63FF)

## About this portfolio

This repository showcases two end-to-end **Power BI portfolio projects** built to transform operational and financial data into clear, actionable dashboards.

The projects demonstrate my ability to work across the full BI workflow:

**Data preparation → Modeling → KPI design → Business analysis → Data visualization → Decision support**

All datasets and business identifiers used here are **anonymized or fictitious** for public demonstration.

---

## Featured projects

| Project | Business focus | What it demonstrates |
|---|---|---|
| **[Financial Pilotage Portfolio](./powerbi-financial-pilotage-portfolio/)** | Financial steering, budget execution, supplier commitments, purchase orders | Financial KPIs, budget tracking, supplier analysis, reconciliation controls |
| **[Procurement & Budget Portfolio](./powerbi-procurement-budget-portfolio/)** | Procurement monitoring, consumables, budget consumption, purchase orders | Procurement analytics, purchasing follow-up, operational reporting, budget monitoring |

---

## 01 — Financial Pilotage Portfolio

**Objective:** provide a clear financial view of commitments, purchasing activity and budget execution.

### Key analysis areas

- Budget consumption and remaining budget
- Purchase-order monitoring
- Supplier commitments
- Ordered vs. received vs. invoiced amounts
- Financial reconciliation and control indicators
- Detailed transactional analysis

### Included files

- **Power BI report:** `dashboard/Portfolio_Pilotage_Financier.pbix`
- **Anonymized dataset:** `data/Portfolio_Commandes_Anonymise.xlsx`
- Project documentation
- SHA-256 integrity checks

**[Explore the Financial Pilotage project →](./powerbi-financial-pilotage-portfolio/)**

---

## 02 — Procurement & Budget Portfolio

**Objective:** turn purchasing and budget data into an easy-to-read operational dashboard.

![Procurement & Budget Dashboard](./powerbi-procurement-budget-portfolio/assets/procurement-budget-consumables.jpg)

> **Visual portfolio:** budget consumption, remaining budget, monthly purchasing trends and procurement KPIs are visible directly in the repository.

### Key analysis areas

- Procurement activity
- Consumables monitoring
- Budget consumption
- Purchase-order tracking
- Operational and financial overview
- Recurring / duration-based purchasing analysis

### Included files

- **Power BI report:** `dashboard/Procurement_Budget_Dashboard.pbix`
- **Anonymized dataset:** `data/Portfolio_Commandes_Anonymise.xlsx`
- Project documentation
- SHA-256 integrity checks

**[Explore the Procurement & Budget project →](./powerbi-procurement-budget-portfolio/)**

---

## Skills demonstrated

### Business Intelligence
- KPI definition and dashboard design
- Financial and procurement analysis
- Data storytelling
- Decision-support reporting
- Business-oriented visualization

### Data & modeling
- Power Query / data transformation
- DAX measures
- Data modeling
- Excel-based data sources
- Data quality and reconciliation checks

### User experience
- Clear information hierarchy
- Executive-level KPI reading
- Operational drill-down
- Simplification of complex financial data

---

## Tech stack

| Tool | Usage |
|---|---|
| **Power BI Desktop** | Data modeling, DAX, dashboards and visual analytics |
| **Power Query** | Data preparation and transformation |
| **DAX** | Business measures and KPI calculations |
| **Microsoft Excel** | Portable anonymized data source |

---

## Repository structure

```text
powerbi-portfolio-projects/
│
├── powerbi-financial-pilotage-portfolio/
│   ├── dashboard/
│   │   └── Portfolio_Pilotage_Financier.pbix
│   ├── data/
│   │   └── Portfolio_Commandes_Anonymise.xlsx
│   ├── README.md
│   └── SHA256SUMS.txt
│
├── powerbi-procurement-budget-portfolio/
│   ├── dashboard/
│   │   └── Procurement_Budget_Dashboard.pbix
│   ├── data/
│   │   └── Portfolio_Commandes_Anonymise.xlsx
│   ├── README.md
│   └── SHA256SUMS.txt
│
└── README.md
```

---

## How to explore the projects

1. Open one of the project folders above.
2. Read the project-specific `README.md`.
3. Download the `.pbix` file.
4. Open it with **Power BI Desktop**.
5. Use the included anonymized Excel file as the portfolio data source if required.

---

## Data privacy

These projects are portfolio versions created for public presentation.

- Business names and identifiers are anonymized or fictitious.
- The datasets are intended for demonstration purposes.
- No confidential production information is intentionally published.

---

## Portfolio focus

My goal with these projects is to demonstrate how I turn complex operational and financial data into dashboards that are:

**Clear · Actionable · Business-oriented · Easy to understand**

---

## Contact

For professional opportunities, collaboration or discussion around **Power BI, Data, Business Analysis or Power Platform**, feel free to connect with me through my GitHub profile.

---

<a id="francais"></a>

# 🇫🇷 Français

> **Projets de Business Intelligence orientés pilotage financier, performance achats et aide à la décision.**

![Power BI](https://img.shields.io/badge/Power%20BI-Data%20Visualization-F2C811?logo=powerbi&logoColor=000)
![Power Query](https://img.shields.io/badge/Power%20Query-ETL-217346)
![DAX](https://img.shields.io/badge/DAX-Analytics-0078D4)
![Excel](https://img.shields.io/badge/Excel-Data%20Source-217346?logo=microsoft-excel&logoColor=white)
![Portfolio](https://img.shields.io/badge/Portfolio-Anonymized-6C63FF)

## À propos de ce portfolio

Ce dépôt présente deux projets **Power BI de bout en bout** conçus pour transformer des données opérationnelles et financières en tableaux de bord clairs, exploitables et orientés décision.

Ces projets illustrent ma capacité à intervenir sur l'ensemble de la chaîne BI :

**Préparation des données → Modélisation → Conception des KPI → Analyse métier → Datavisualisation → Aide à la décision**

Toutes les données et tous les identifiants métier présents dans ce portfolio sont **anonymisés ou fictifs** afin de permettre une présentation publique.

---

## Projets principaux

| Projet | Enjeu métier | Compétences démontrées |
|---|---|---|
| **[Portfolio Pilotage Financier](./powerbi-financial-pilotage-portfolio/)** | Pilotage financier, exécution budgétaire, engagements fournisseurs, commandes | KPI financiers, suivi budgétaire, analyse fournisseurs, contrôles de cohérence |
| **[Portfolio Achats & Budget](./powerbi-procurement-budget-portfolio/)** | Suivi des achats, consommables, consommation budgétaire, commandes | Analyse achats, suivi opérationnel, reporting, pilotage budgétaire |

---

## 01 — Portfolio Pilotage Financier

**Objectif :** fournir une vision claire des engagements, de l'activité achats et de l'exécution budgétaire.

### Principaux axes d'analyse

- Consommation et budget restant
- Suivi des commandes
- Engagements fournisseurs
- Comparaison commandé / réceptionné / facturé
- Indicateurs de rapprochement et de contrôle financier
- Analyse détaillée des transactions

### Fichiers inclus

- **Rapport Power BI :** `dashboard/Portfolio_Pilotage_Financier.pbix`
- **Jeu de données anonymisé :** `data/Portfolio_Commandes_Anonymise.xlsx`
- Documentation du projet
- Contrôles d'intégrité SHA-256

**[Découvrir le projet Pilotage Financier →](./powerbi-financial-pilotage-portfolio/)**

---

## 02 — Portfolio Achats & Budget

**Objectif :** transformer des données achats et budgétaires en un tableau de bord opérationnel simple à lire.

![Dashboard Achats & Budget](./powerbi-procurement-budget-portfolio/assets/procurement-budget-consumables.jpg)

> **Aperçu visuel :** consommation budgétaire, budget restant, tendances mensuelles et KPI achats sont visibles directement depuis le dépôt.

### Principaux axes d'analyse

- Activité achats
- Suivi des consommables
- Consommation budgétaire
- Suivi des commandes
- Vision opérationnelle et financière
- Analyse des achats récurrents ou de longue durée

### Fichiers inclus

- **Rapport Power BI :** `dashboard/Procurement_Budget_Dashboard.pbix`
- **Jeu de données anonymisé :** `data/Portfolio_Commandes_Anonymise.xlsx`
- Documentation du projet
- Contrôles d'intégrité SHA-256

**[Découvrir le projet Achats & Budget →](./powerbi-procurement-budget-portfolio/)**

---

## Compétences démontrées

### Business Intelligence
- Définition de KPI et conception de tableaux de bord
- Analyse financière et achats
- Data storytelling
- Reporting d'aide à la décision
- Visualisation orientée métier

### Données & modélisation
- Power Query / transformation des données
- Mesures DAX
- Modélisation de données
- Sources Excel
- Contrôles qualité et rapprochements

### Expérience utilisateur
- Hiérarchie claire de l'information
- Lecture rapide des KPI de pilotage
- Analyse détaillée opérationnelle
- Simplification de données financières complexes

---

## Stack technique

| Outil | Utilisation |
|---|---|
| **Power BI Desktop** | Modélisation, DAX, dashboards et analyses visuelles |
| **Power Query** | Préparation et transformation des données |
| **DAX** | Mesures métier et calculs de KPI |
| **Microsoft Excel** | Source de données portable et anonymisée |

---

## Structure du dépôt

```text
powerbi-portfolio-projects/
│
├── powerbi-financial-pilotage-portfolio/
│   ├── dashboard/
│   │   └── Portfolio_Pilotage_Financier.pbix
│   ├── data/
│   │   └── Portfolio_Commandes_Anonymise.xlsx
│   ├── README.md
│   └── SHA256SUMS.txt
│
├── powerbi-procurement-budget-portfolio/
│   ├── dashboard/
│   │   └── Procurement_Budget_Dashboard.pbix
│   ├── data/
│   │   └── Portfolio_Commandes_Anonymise.xlsx
│   ├── README.md
│   └── SHA256SUMS.txt
│
└── README.md
```

---

## Comment explorer les projets

1. Ouvrir l'un des dossiers projet ci-dessus.
2. Consulter le `README.md` associé.
3. Télécharger le fichier `.pbix`.
4. L'ouvrir avec **Power BI Desktop**.
5. Utiliser le fichier Excel anonymisé inclus comme source portfolio si nécessaire.

---

## Confidentialité des données

Ces projets sont des versions portfolio créées pour une présentation publique.

- Les noms d'entreprises et identifiants métier sont anonymisés ou fictifs.
- Les jeux de données sont fournis uniquement à des fins de démonstration.
- Aucune information confidentielle de production n'est volontairement publiée.

---

## Objectif du portfolio

L'objectif de ces projets est de montrer ma capacité à transformer des données opérationnelles et financières complexes en tableaux de bord :

**Clairs · Exploitables · Orientés métier · Faciles à comprendre**

---

## Contact

Pour toute opportunité professionnelle, collaboration ou échange autour de **Power BI, la Data, la Business Analysis ou Power Platform**, vous pouvez me contacter via mon profil GitHub.
